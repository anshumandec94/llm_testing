...
2. **Decide `maxlen` deliberately** before the real runs, knowing 200 discards 39% of the data and 1000 costs about 25 times the attention compute of 200. **This is now largely superseded by issue #24 (sliding windows):** once long histories yield several contiguous training windows instead of one truncated tail, `maxlen` governs only inference context and attention cost, not how much data training sees. That decouples the two and makes a moderate `maxlen` cheap in accuracy terms. **#24 landed on 2026-09-24 (PR #29)**: at `maxlen=200` and the default stride, training now sees 100% of interactions through 287,811 windows. **Anshuman decided on 2026-09-25 to keep `maxlen = 200`.** Longer histories are used for training as consecutive windows of up to 200, so nothing is discarded, and inference sees each user's most recent 200 training ratings.
